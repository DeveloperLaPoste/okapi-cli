...
const processCmd = (args, stdinContent, cb) => Promise.resolve()
  .then(() => {
    ...
    if (typeof argv.env !== 'undefined') {
      if (argv.env) {
        ...
        if (envName) {
          ...
          print.err(chalk.red.bold(`environment ${argv.env} not supported`));
          return exit(1);
        }
      }
      if (argv._.length < 1) {
    ...
    if (typeof argv.baseurl !== 'undefined') {
      if (argv.baseurl === '') {
        print.out(chalk.cyan.bold(curEnv.baseUrl || settings.value.baseUris[envName] || 'https://api.laposte.fr'));
        return exit();
      }
    ...
    if (argv.tocurl) {
      ...
      curlCmd.push([okaInfo.auth && `-u "${okaInfo.auth.user}:${okaInfo.auth.pass}"`]);
      if (okaInfo.form) {
    ...
    if (typeof oka[method] !== 'function') {
      print.err(chalk.red.bold(`method ${method} is not supported`));
      return exit(1);
    }
    return oka[method]()
      .spread((data, res) => {
...
exports = module.exports = (text, opt, cb) => Promise.resolve()
  .then(() => {
    ...
    return processCmd(_.compact(text.split(' ').map(line => line.replace(/^['"]?(.*)['"]?$/, '$1'))));
  })
